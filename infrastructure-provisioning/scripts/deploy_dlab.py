...
def deploy_dlab(args):
    ...
    deploy_params = 'sudo docker run -i -v {0}{1}.pem:/root/keys/{1}.pem -v {2}/web_app:/root/web_app '\
                    ...
                    '-e "conf_resource={22}" -e "aws_openldap_instance_size=t2.medium" -e "aws_region={5}" ' \
                    ...
                    '-e "ldap_domain={18}" -e "ldap_host={19}" -e "deploy_ldap={20}" docker.dlab-{22} --action {21}' \
                    .format(args.key_path, args.key_name, args.workspace_path, args.os_family, args.cloud_provider,
                            ...
                            args.ldap_domain, args.ldap_host, args.openldap.lower(), args.action, '{0}')
    print(deploy_params)
    # Creating SSN node
