...
parser.add_argument('--aws_billing_bucket', type=str, default='',
                    help='The name of S3 bucket where billing reports will be placed.')
parser.add_argument('--aws_report_path', type=str, default='',
...
parser.add_argument('--openldap', type=str, default='false', help='If true, deploy OpenLDAP')
parser.add_argument('--ldap_adm_user', type=str, help='LDAP admin username')
parser.add_argument('--ldap_adm_pass', type=str, help='LDAP admin password')
...
parser.add_argument('--ldap_test_user', type=str, default='false', help='If true, add test user to LDAP')
...
def deploy_dlab(args):
    ...
    deploy_params = 'sudo docker run -i -v {0}{1}.pem:/root/keys/{1}.pem -v {2}/web_app:/root/web_app '\
                    '-e "conf_key_name={1}" -e "conf_os_family={3}" -e "conf_cloud_provider={4}" ' \
                    '-e "conf_resource={23}" -e "aws_openldap_instance_size=t2.medium" -e "aws_region={5}" ' \
                    '-e "aws_vpc_id={6}" -e "aws_subnet_id={7}" -e "aws_security_groups_ids={8}" ' \
                    '-e "conf_service_base_name={9}" -e "aws_access_key={10}" -e "aws_secret_access_key={11}" ' \
                    '-e "conf_tag_resource_id={12}" -e "aws_account_id={13}" -e "aws_billing_bucket={14}" ' \
                    '-e "aws_report_path={15}" -e "ldap_adm_user={16}" -e "ldap_adm_pass={17}" ' \
                    '-e "ldap_domain={18}" -e "ldap_host={19}" -e "deploy_ldap={20}" -e "ldap_test_user={21}" ' \
                    'docker.dlab-{23} --action {22}' \
                    .format(args.key_path, args.key_name, args.workspace_path, args.os_family, args.cloud_provider,
                            args.region, args.vpc_id, args.subnet_id, args.sg_ids, args.infrastructure_tag,
                            args.access_key_id, args.secret_access_key, args.tag_resource_id, args.aws_account_id,
                            args.aws_billing_bucket, args.aws_report_path, args.ldap_adm_user, args.ldap_adm_pass,
                            args.ldap_domain, args.ldap_host, args.openldap.lower(), args.ldap_test_user.lower(),
                            args.action, '{0}')
    print(deploy_params)
    # Creating SSN node
    local(deploy_params.format('ssn'))
    if args.openldap.lower() == 'true':
        # Creating OpenLDAP node
        local(deploy_params.format('openldap'))
